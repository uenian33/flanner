...
SHEET_JS = """
  ...
  /* ---- the plan is yours, and the screen says so ----
     ...
     The motion is the spec's, by token rather than by feel: the fall takes an
     extra-long duration — 700 to 1000, which is extra-long1 to extra-long4,
     varied per slip so they do not land together — it leaves on the
     emphasised accelerate curve, because a thing on its way off the screen
     accelerates out of it, it crosses on the standard curve, and the slips
     start short2 apart, 50ms, so the shower reads as one event rather than as
     a queue. Nothing here is a number that felt right. */
  celebratePicks() {
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    ...
    document.body.appendChild(layer);
    const N = 22, pal = this.STAGE_PALETTE.length;
    /* The motion tokens, by name: emphasised accelerate for the way out,
       standard for the crossing, extra-long for the fall, short2 between the
       slips. */
    const ACCEL = 'cubic-bezier(0.3, 0, 0.8, 0.15)';
    const STD = 'cubic-bezier(0.2, 0, 0, 1)';
    for (let i = 0; i < N; i++) {
      const spin = i % 2 ? 1 : -1;
      const w = 6 + (i % 3) * 3, h = 10 + (i % 4) * 4;
      const x = ((i + .5) / N) * W + ((i * 37) % 23) - 11;
      const p = document.createElement('i');
      p.style.cssText = 'position:absolute;left:' + x.toFixed(1) + 'px;top:0;'
        + 'width:' + w + 'px;height:' + h + 'px;border-radius:2px;'
        + 'background:' + this.stageColor(i % pal).dot + ';will-change:transform,opacity';
      layer.appendChild(p);
      /* Out of sight above the edge, across the screen, out of sight below —
         with a drift each way, so no two fall on the same line. */
      const drift = ((i % 5) - 2) * 26;
      p.animate([
        { transform: 'translate3d(0,' + (-40 - (i % 6) * 24) + 'px,0) rotate(0deg)',
          opacity: 1, easing: ACCEL },
        { offset: .55, opacity: 1,
          transform: 'translate3d(' + (drift * .6).toFixed(1) + 'px,'
            + (H * .55).toFixed(0) + 'px,0) rotate(' + (220 * spin) + 'deg)',
          easing: STD },
        { transform: 'translate3d(' + drift.toFixed(1) + 'px,' + (H + 60) + 'px,0) '
            + 'rotate(' + (540 * spin) + 'deg)', opacity: 0 }
      ], {
    ...
    this.paperT = setTimeout(() => layer.remove(), 1500);
  }
