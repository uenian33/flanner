...
SHEET_JS = """
  ...
  rowFlyGhost(host) {
    /* The picture, and only the picture. A row is the width of the screen and
       a thing that wide drawn through a 40px button is a curtain closing; the
       row cut down to its picture and its name was better and still not
       right, because the name has to be squeezed to reach a round button and
       squeezing a word is the one thing that reads as a trick. The picture is
       already the act, already square, and already the shape of the thing it
       is going into — so it goes on its own, at its own proportions the whole
       way. */
    const art = host.firstElementChild;
    if (!art) return null;
    ...
    g.style.cssText = 'position:fixed;margin:0;z-index:94;pointer-events:none;'
      + 'left:' + ab.left.toFixed(1) + 'px;top:' + ab.top.toFixed(1) + 'px;'
      + 'width:' + ab.width.toFixed(1) + 'px;height:' + ab.height.toFixed(1) + 'px;'
  ...
  flyToPlan(host, prebuilt) {
    ...
    /* A picture keeps its proportions. The stretch-and-neck is a minimise, and
       a minimise is for a thing whose shape is arbitrary — a window, a cell of
       a timetable. A picture is not that: warping it reads as the picture
       being wrong rather than as the picture being put away. So the one that
       leaves a row travels the same eased arc at the same eased speed and only
       gets smaller, on one scale for both axes. */
    const plain = ghost.hasAttribute && ghost.hasAttribute('data-fp-plain');
    const endEven = Math.max(.05, Math.min(to.width / Math.max(from.width, 1),
      to.height / Math.max(from.height, 1)) * .82);
    const N = 26, frames = [];
    for (let i = 0; i <= N; i++) {
      const t = i / N, p = move(t), q = 1 - p;
      const x = 2 * q * p * cx + p * p * dx;
      const y = 2 * q * p * cy + p * p * dy;
      if (plain) {
        frames.push({
          offset: t, easing: 'linear',
          opacity: t < .78 ? 1 : Math.max(0, 1 - (t - .78) / .22),
          transform: 'translate3d(' + x.toFixed(1) + 'px,' + y.toFixed(1) + 'px,0) '
            + 'scale(' + (1 + (endEven - 1) * shrink(t)).toFixed(4) + ')'
        });
        continue;
      }
...
def patch_sheet(src: str) -> str:
    ...
    src = sub_once(
        ...
        "          return;\n"
        "        }\n"
        "        /* setSheetEl plays it in — one place, or it rises twice. */\n"
        "        this.setSheetEl(el);\n"
        "      };\n"
        "      wire(6);\n"
...
def patch_nav(src: str) -> str:
    ...
    src = sub_once(
        ...
        "      /* A binding, not a getter on the class: `sc-if` reads the view\n"
        "         object the template is rendered against, so a name that is only\n"
        "         on the logic resolves to nothing and the section disappears. */\n"
        "      hasLineup: this.LINEUP.length > 0,\n"
    ...
    src = sub_once(
        ...
        "       a row is the row copied and cut at its controls: the picture\n"
        "       and what it says, and none of the buttons. Both take the same\n"
        "       arc from there. */\n"
        "    const host = srcEl && srcEl.closest\n"
        "      ? srcEl.closest('[role=\"button\"]') : null;\n"
        "    if (host && !this.state.star[id]) {\n"
        "      if (host.closest('li')) {\n"
        "        const g = this.rowFlyGhost(host);\n"
...
_THEME_ASSET: tuple[str, dict] | None = None


def theme_asset(recipe: dict) -> str:
    """The colour engine and the recipe, as one file the whole site shares.

    Neither half depends on which festival is being built — the arithmetic is
    the arithmetic, and the recipe is the design's own tones with the hue taken
    out — so this is written once and named by a hash of itself. A planner links
    it and hands it an accent.

    The recipe is handed in rather than derived here, and it is the same object
    the page's own stylesheet was rendered from: one source for the colour a
    build writes and the colour a browser writes, so the two cannot drift into
    disagreeing about a festival. That it really is festival-independent is not
    assumed — the second planner to ask for it is checked against the first, and
    a difference fails the build rather than shipping one planner a file built
    for another.
    """
    global _THEME_ASSET
    if _THEME_ASSET:
        url, first = _THEME_ASSET
        if recipe != first:
            names = {e["n"] for e in first["light"]} ^ {e["n"] for e in recipe["light"]}
            raise MissingAnchor(
                "the theme recipe is not the same for every festival — it is "
                "shipped as one shared file, so it has to be. Differing tokens: "
                + (", ".join(sorted(names)) or "same names, different values"))
        return url
    src = ("/* Generated by build_planner.py — the recipe, then the engine. */\n"
           "window.__FP_THEME_RECIPE__ = %s;\n%s"
           % (json.dumps(recipe, separators=(",", ":")),
              (ROOT / "scripts" / "_theme.js").read_text()))
    data = src.encode()
    name = "theme-%s.js" % hashlib.sha256(data).hexdigest()[:10]
    out = ROOT / "assets" / "js" / name
    if not out.exists() or out.read_bytes() != data:
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_bytes(data)
    _THEME_ASSET = ("../assets/js/" + name, recipe)
    return _THEME_ASSET[0]
...
/* Three lines, and three whole ones. The height is three of the paragraph's
   line boxes — but `em` resolves against the box's own font-size, and the box
   inherited the card's 15px while the paragraph is set at 13, so the window
   was 65.25px over an 18.85px line: three lines and 46% of a fourth, sliced
   through the middle of its letters. The size is restated here so `1.45em` is
   the line it is counting. What decides whether there is a Read more at all
   already measures the paragraph's own leading; this is the box catching up
   with it. */
.fest .about__clip { overflow: hidden; font-size: 13px; max-block-size: calc(3 * 1.45em); transition: max-block-size 340ms var(--ease); }
...
def build(fid: str) -> pathlib.Path:
    ...
    theme_js = theme_asset(recipe)

    page = f"""<!doctype html>
<!-- The festival this page is, in the two facts the theme needs. The
     scheme below is already written at this accent, so the first paint is
     themed; these are what let the same engine re-derive it at runtime for
     a festival the build never saw. -->
...
{art.lib_tags()}
<!-- The colour engine and the design's own tones with the hue taken out of
