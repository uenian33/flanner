...
SHEET_JS = """
  ...
  rowFlyGhost(host) {
    ...
    /* The row copied whole and then cut, rather than the picture and the name
       laid out again beside each other. Laid out again they land somewhere
       else: a row sets its name on the first of three lines and the copy
       centred it against the picture, so the moment the copy appeared the
       name dropped half its height. A clone at the row's own width has the
       row's own layout by construction — every glyph on the pixel it was on —
       and the cut is a window over it, at the edge where the trailing
       controls begin. Those are what the copy leaves behind: the star is the
       thing being pressed and the arrow goes nowhere. */
    const kids = host.children;
    const tail = kids.length ? kids[kids.length - 1].getBoundingClientRect() : null;
    const cut = tail && tail.width ? Math.max(64, tail.left - rb.left - 4) : rb.width;
    const wrap = document.createElement('div');
    wrap.setAttribute('aria-hidden', 'true');
    wrap.style.cssText = 'position:fixed;margin:0;z-index:94;pointer-events:none;'
      + 'left:' + rb.left.toFixed(1) + 'px;top:' + rb.top.toFixed(1) + 'px;'
      + 'width:' + cut.toFixed(1) + 'px;height:' + rb.height.toFixed(1) + 'px;'
      + 'border-radius:' + getComputedStyle(host).borderRadius + ';overflow:hidden;'
    ...
    copy.style.left = '0';
    copy.style.top = '0';
    copy.style.width = rb.width.toFixed(1) + 'px';
    copy.style.height = rb.height.toFixed(1) + 'px';
