...
# The star's own fill, as a (chroma, tone) recipe rather than a role.
#
# It was `primary-container`, and that role takes as much chroma as the source
# has: at tone 90 the sRGB gamut is enormous through the yellows and greens, so
# Kallio's lime came out at chroma 78 and glared off the page while the same
# role gave the Helsinki Festival a chroma-13 salmon. One rule, two wildly
# different results, because the rule was not capping anything.
#
# A planner's own page does not have this problem, because it re-tones a design
# token and only the hue turns — its picked star measures #e0e6d2, chroma 10.6
# at tone 90, whatever festival it is. So this caps the same way `ART_TONES`
# does, at that chroma, and every festival's star now sits within a few units
# of the planner's own. Capped by `min`, so a festival with less chroma than
# this keeps its own rather than being given colour it has not got.
#
#            light (chroma, tone)   dark (chroma, tone)
PICK_BG = ((11, 90), (11, 30))
...
def artwork_css(cfg: dict) -> str:
    ...
    def rule(sel: str, src: str, dark: bool, prefix: str) -> str:
        ...
        pc, pt = PICK_BG[1 if dark else 0]
        vals += (f";--pick-bg:{m3color.tone(hue, min(pc, chroma), pt)}"
                 f";--pick-fg:{s['primary']}")
...
def highlight_css(cfg: dict) -> str:
    ...
    def rule(sel: str, src: str, dark: bool, prefix: str) -> str:
        ...
        # The star's fill is a recipe, not a role — see PICK_BG.
        hue, chroma = _hue_chroma(src)
        pc, pt = PICK_BG[1 if dark else 0]
        vals += f";--pick-bg:{m3color.tone(hue, min(pc, chroma), pt)}"
