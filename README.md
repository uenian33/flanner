...
| The forecast, in the festival's hue | The forecast had a colour of its own — a pink, picked against the green — and dynamic colour would put a role like that at tertiary, hue + 60. It goes to the festival's own hue instead: one hue for the whole page reads as one page, where a forecast sixty degrees off the festival it belongs to read as something borrowed from another app. It keeps its own tones, so it is still a surface of its own rather than the About card again, and the temperature in the bar follows because it is drawn from the same six. The names stay `--pink-*`, which every use site says. |
