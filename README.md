...
| What you starred goes where it is kept | Pressing the star turned a cell dark and put a number up somewhere else on the screen, with nothing between them the eye could follow — so the cell goes: a copy leaves the grid on the card's own arc, stretches along the line of travel, necks down across it and is drawn through the button that counts your picks, which answers with a beat of its own. A row in the list sent nothing, and for a reason: it is the width of the screen, and a thing that wide drawn through a 40px button is a curtain closing rather than an act being put somewhere. So a row sends the row copied and cut: a clone at the row's own width, inside a window that ends where the trailing controls begin, on the same arc. Copied rather than rebuilt — laying the picture and the name out again put the name somewhere else, because a row sets it on the first of three lines and a fresh two-item row centres it, so the name dropped half its height the moment the copy appeared. A clone has the row's own layout by construction, every glyph on the pixel it was on. Only starring flies; unstarring does not, and neither does anything under reduced motion. |
...
| The same throw on the list page | Starring a festival here does what starring an act does in a planner: a copy leaves the page on the card system's arc, stretches along the line of travel, necks across it and is drawn through **My plan**, whose icon answers out and back on the emphasised curve and whose count arrives a beat later. What travels depends on the shape it leaves — a tile and the highlight go whole, because they are cards and a card is the thing being put away; a row is copied at its own width and shown through a window that ends where its star begins, since a thing the width of the screen drawn through a 54px cell is a curtain closing. The count is M3's badge, in `error` — the one role the monochrome variant keeps, semantic colours being exempt by construction, so the page gets a highlight colour without getting a hue that belongs to no festival on it. It follows the cell that is showing: the shell's dock below the rail's width and the rail's own above it, put back whenever the dock repaints. Written only when it changes — setting the text replaces a node, which is a change on the row being watched for exactly that repaint, and a badge that rewrote itself every time hung the page. |
