...
| | |
|---|---|
| The pictures you cannot see | The add-to-home card's three screenshots are of Safari's share sheet, so they are shown only on iOS — everywhere else the menu item is one level down and named on the tin, and Safari's pictures would be the wrong instructions. But the card only appears below 600px, so on any desktop browser there was no way to look at them at all, and they were reported missing twice by someone who was not on a phone. `?a2hs=ios` forces the illustrated steps and `?a2hs=other` the worded ones, so either can be checked without a phone in your hand. |
| The blast belongs to the card it came from | Starring a festival throws the planner's own blast — two rings, streaks, sparks and stars — and its five colours are read off the pressed element rather than written down. But two of the five were `--accent` and `--chip`, which only the highlight declares: on a list card they fell through to the page's own monochrome, so fourteen of every thirty-two sparks came out black and seven grey. They are five that `artwork_css` declares on the bare `[data-id]` now, which every themed surface matches, and their shape is the planner's — its sparks are the stage's dot, its ground and its plan pair, and these are the same four steps of the festival's ramp. Measured on Kallio's row: `#436900` ×14, `#b8d189` ×7, `#dfe5d0` ×7, `#50662b` ×2, and the two rings on the accent. |
| The star went tonal | A starred festival wore its colour as a saturated disc with a white star on it — nine of those down the edge of a list read as a colour chart rather than as a list with some things starred in it. It is a filled *tonal* icon button now, which is M3's quieter one: the container is the fill and the accent is the icon on it, so the star is the bright thing and the disc behind it is the festival's colour gone quiet. Its own pair (`--pick-bg`/`--pick-fg`), not the `--sel-*` the highlight's segmented button uses, because those are two different M3 components and only one of them is filled. The fill is a `(chroma, tone)` recipe rather than a role, for a reason worth stating: `primary-container` takes as much chroma as its source has, and at tone 90 the sRGB gamut is enormous through the yellows and greens — so one rule gave Kallio a chroma-78 lime that glared off the page and the Helsinki Festival a chroma-13 salmon that did not. A planner's own page never has this problem, because it re-tones a design token and only the hue turns: its picked star is `#e0e6d2`, chroma 10.6, whichever festival it is. So `PICK_BG` caps at that, the way `ART_TONES` already caps, and the nine now span chroma 8.5 to 15.0 against the planner's 10.6. Accent on fill is 4.98:1 at worst across both themes, against a 3:1 bar for an icon. |
| Two festivals gave way | Three pairs generated the same primary, and no algorithm could resolve it: each was the colour its festival actually publishes. Lost In Music moved 41° off the yellow-green it shared with Flow and Kallio, and Espoo Ciné 27° off the violet it shared with Love & Anarchy. Both were chosen by searching every hue for the one that clears the threshold by the widest *margin* while staying nearest home, not the one that merely clears it — the first answer the solver offered moved Lost In Music only 4° and paid for it by collapsing chroma 55.6 → 25.5, because the sRGB yellow gamut peaks sharply at h105 and falls off a cliff either side. A pale card is not a distinct card. At h150 it clears at 1.59× and is still a green rather than a full teal. |
...
| Instructions you can throw away | The add-to-home-screen card takes the gesture an act's card takes, and by the same rule for whose it is: while the words are at their top there is nothing below to scroll to, so a downward drag anywhere on the card is the card's; once they are scrolled, the same drag is theirs to scroll back with. Touch events rather than pointer events, and not passive, because that is the only way to take the gesture back — `touch-action: pan-y` hands a vertical drag to the scroller before a pointermove ever arrives, the scroller at its top does nothing with it, and the browser then cancels our pointer. Calling preventDefault on the first move of a drag that belongs to the card stops that, and from there the card follows the finger exactly: every move is one transform, no threshold to make up and nothing in between. It follows the finger, the scrim fades with it, and on release it is thrown out past 28% of its height or a flick over 600px a second, and springs back otherwise. However it is closed — the button, the scrim, Escape, or the drag — it leaves the way it arrived, back down to the edge it rose from, and quicker going than coming: 280ms on the accelerate against 420 on the emphasised decelerate, which is M3's rule for an exit. The phone's alone: above that width this is a dialog in the middle of the screen, with no edge it would be dragged back to. Whatever a drag left behind is cleared on the way in as well as out — a card reopened still wearing the transform it was thrown out with is a card that never appears again. |
| The sheet opens, and nothing is ringed | A modal dialog focuses its first focusable descendant, and in the settings card that is the close button — so on a real phone the card rose with a focus ring drawn round the cross, a circle at the top corner that belongs to no state this button has. WebKit counts the focus a dialog moves for you as keyboard focus, so `:focus-visible` matched something nobody had reached for. The sheet itself takes that focus instead (`tabindex="-1" autofocus` on the box, and no outline on a container that is only holding it): a screen reader still enters the card at its title, Tab still reaches the close button and still rings it, and the ring is once again only ever the answer to a key. The controls in the card also refuse iOS's own tap highlight, which follows `border-radius` and would paint the same disc grey for the length of a press, and their hover states are behind `@media(hover:hover) and (pointer:fine)` so a tap does not leave one behind. |
| The highlight sends its heart | The card itself does not throw — one card filling most of the screen flying to the bar reads as the page leaving rather than a festival being put away. The heart that was just pressed is another matter: it is small, and a small thing can cross a screen without the page seeming to move with it. It keeps its size the whole way, where everything else that travels shrinks — a card has to become small enough to enter a 54px cell, and a heart already is that size, so shrinking it would only make it harder to follow. What is left is the path and the speed: the same eased arc up and over, the same eased travel along it, a fade where it arrives, and the cell's own beat to take it in. |
| The highlight follows the finger | It compared two clientX values and, on release, decided whether that had been a swipe — so nothing moved until the finger was lifted and then a card slid on its own. What a reader is doing when they drag is moving the thing they are touching, and everything about whether it will go is in how far it has come. The track is offset by the distance travelled while the finger is down, and on release it carries on to the next card or falls back to the one it left: a quarter of the card's width, or a flick over 500px a second that has also travelled 24 — speed alone would let a five-pixel twitch turn the page. Past either end it resists rather than showing the blank beyond the last card. The gesture only takes over once it is clearly sideways, so a finger that starts on the card and goes down is still a page scroll. |
| A way out of an empty plan | The empty plan says to star a festival, and now offers the button that gets you to some: M3's filled button, which on the monochrome scheme is ink on paper rather than a hue belonging to no festival on the page. It presses the Festivals destination rather than being a second route to the same place. Only when the plan is empty — where a filter has merely narrowed a plan that has something in it, the way out is to widen the filter, not to leave. |
| Where, instead of how | On a phone the row's third control is the place a festival is in rather than the two shapes a list can take. Which shape you like is set once and never touched; where something is on is the question a phone is most often asking, and the switcher was taking the width to answer it. Every width above the phone's keeps the switcher. The list is derived from the records — a festival says "Suvilahti, Helsinki", a venue and the city it is in, so the city is the last part of it — and it opens on Everywhere, because a filter that quietly hides two thirds of the page on arrival is a bug report waiting to happen. `Use my location` asks the browser once, compares the answer against the cities the site covers and picks the nearest; nothing is stored and nothing leaves the page. A city with no coordinate can still be chosen by hand — it just cannot be the answer to that question, and the sheet says which it was rather than choosing something else. |
