...
| The forecast, at tertiary | The forecast is the one thing on the page that is neither the festival nor an act, and the design gave it a colour of its own — a pink, picked against the green. Under dynamic colour that role is tertiary, which M3 derives from the source at hue + 60, so the whole family goes there: the same tones it was drawn in, at the hue the festival's own colour puts it. Flow's forecast is green, Kallio's is teal, and the temperature in the bar follows because it is drawn from the same six. The names stay `--pink-*`, which every use site says. |
